...
namespace PushSharp
{
    public class PushBroker : IPushBroker
    {
        ...
        public void StopAllServices(CancellationToken token, bool waitForQueuesToFinish = true)
        {
            ...
            stopping.AsParallel().ForAll(sr => StopService(sr, token, waitForQueuesToFinish));
        }
        ...
        public void StopAllServices<TNotification>(CancellationToken token, bool waitForQueuesToFinish = true)
        {
            StopAllServices<TNotification>(null, token, waitForQueuesToFinish);
        }
        ...
        public void StopAllServices<TNotification>(string applicationId, CancellationToken token, bool waitForQueuesToFinish = true)
        {
            var type = typeof(TNotification);
...
&& s.ApplicationId.Equals(applicationId)
        ...
        /// <param name="applicationId">Application identifier.</param>
        /// <param name="waitForQueuesToFinish">If set to <c>true</c> waits for queues to be drained before returning.</param>
        public void StopAllServices(string applicationId, CancellationToken token, bool waitForQueuesToFinish = true)
        {
            var stopping = new List<ServiceRegistration>();
        ...
        void StopService(ServiceRegistration sr, CancellationToken token, bool waitForQueuesToFinish)
        {
            sr.Service.Stop(token, waitForQueuesToFinish);
        ...
        {
            StopAllServices(CancellationToken.None,false);
        }
    }
}
